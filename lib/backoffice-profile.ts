...
const MEMBER_TIERS = new Set(['MODULE', 'STARTER', 'ENTRY', 'FOUNDATION', 'TEST_MODULE'])

function tierToRole(tier: string | null | undefined): UserRole {
  if (!tier) return 'MEMBER'
  if (VIP_TIERS.has(tier)) return 'VIP'
  if (MEMBER_TIERS.has(tier)) return 'MEMBER'
  return 'MEMBER'
}
...
export async function ensureUserProfile(
  ...
  if (existing?.referral_code) {
    ...
    if (typeof params?.email === 'string' && params.email && params.email !== existing.email) {
      patch.email = params.email
    }

    if (typeof params?.walletAddress === 'string' && params.walletAddress && params.walletAddress !== existing.wallet_address) {
      patch.wallet_address = params.walletAddress
    }

    if (params?.tier && params.tier !== existing.current_tier) {
      patch.current_tier = params.tier
      patch.role = tierToRole(params.tier)
    }
...
export async function syncUserProfileFromPayment(privyUserId: string, tier: string) {
  return ensureUserProfile(privyUserId, { tier })
}
