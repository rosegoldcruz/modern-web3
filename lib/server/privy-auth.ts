...
function extractEmail(user: unknown): string | null {
  ...
  if (typeof maybeUser.email?.address === 'string') {
    return maybeUser.email.address
  }
  ...
  for (const account of linked) {
    ...
    if (account?.type === 'email' && typeof account.email === 'string') {
...
function extractWalletAddress(user: unknown): string | null {
  ...
  if (typeof maybeUser.wallet?.address === 'string') {
    return maybeUser.wallet.address
  }

  const linked = maybeUser.linkedAccounts ?? maybeUser.linked_accounts ?? []
  for (const account of linked) {
    if ((account?.type === 'wallet' || account?.type === 'smart_wallet') && typeof account.address === 'string') {
      return account.address
    }
  }
